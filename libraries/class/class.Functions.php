<?php
	class Functions
	{
		...
		function showProduct($v, $options = array(), $k)
		{

			if (!isset($options['class'])) {
				$options['class'] = 'col-12 col-sm-6 col-md-4 col-lg-4 item-product	';
			}
			global $config_url, $lang,$sluglang;
			...
			if ($v['spmoi'] == 1) {
				echo '<div class="rb-new hide"></div>';
			}
			if ($v['khuyenmai']) {
				echo '<div class="rb-promotion hide"></div>';
			}
			echo '<div class="xm-image"><div class="x-inner"><a href="' . $v[$sluglang] . '" title="' . $v['ten_' . $lang] . '" class=""><img src="'.WATERMARK.'/product/350x350x1/'.UPLOAD_PRODUCT_L.$v['photo'].'" 
			class="img-fluid lazy" alt="' . $v['ten' . $lang] . '" /><div class="hover-shine"></div></a></div></div>';

			echo '<div class="tools">';
			echo '<div class="wrap-tools"><div class="inner-tools">';
			echo '<div class="clearfix"></div>';
			echo '<div class="name ">';
			echo '<h2><a href="' . $v[$sluglang] . '" title="' . $v['ten_' . $lang] . '" class="">' . ($v['ten' . $lang]) . '</a></h2>';
			echo '</div>';
			echo '<div class="clearfix"></div>';
			echo '<div class="wrap-price">';
			echo '<div class="price ">';
			echo '<span class="red">' . (($v['gia']) ? $this->format_money($v['gia']) : _gia . ': <a href="lien-he.html" title="Liên hệ">Liên hệ</a></span>') . '</span>';
			echo '</div>';
			if (($v['gia'] > 0) & ($v['gia'] > $v['giamoi']) & $v['giamoi'] > 0) {
				echo '<div class="old-price pull-left">';
				echo '<span class="blue">' . $this->format_money($v['giamoi']) . '</span>';
				echo '</div>';
			}

			echo '</div><!-- end wrap-price -->';
			echo '<div class="clearfix"></div></div><!--end --></div></div><!-- end inner-tools--> <!-- end tools-->';

			echo '<div class="product-desc anim-05">';
			echo '<h2><a href="' . $v[$sluglang] . '" title="' . $v['ten' . $lang] . '" class="">' . $v['ten' . $lang] . '</a></h2>';
			echo $this->cutString(strip_tags($v['mota' . $lang]));
			echo '</div>';
			echo '</div>';
			echo '</div></div>';
			...
			echo '<h2><a href="' . $v['tenkhongdau'] . '" title="' . $v['ten' . $lang] . '" class="">' . $v['ten_' . $lang] . '</a></h2>';
			echo '</div>';
			echo '<div class="product-price hide">';
			echo '<div class="pull-right add-cart">';
			echo '<span class="cart-add addcart transition" data-id="'.['id'].'" data-action="addnow">Thêm vào giỏ hàng</span>
			<span class="cart-buy addcart transition" data-id="'.['id'].'" data-action="buynow">Mua ngay</span>
			';
			echo '</div>';
			echo '<div class="clearfix"></div>';
			echo '</div>';
