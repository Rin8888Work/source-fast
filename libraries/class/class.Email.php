<?php
	...
	class Email
	{
		...
		public function infoEmail()
		{
			...
			$social = array();
			$socialString = '';
			$this->company = $this->d->rawQueryOne("select options, tenvi,diachivi from #_setting limit 0,1");
			...
			$this->data['email'] = $this->optcompany['email'];
			...
			$this->data['company:address'] = $this->company['diachivi'];
		...
		public function sendEmail($owner='', $arrayEmail=array(), $subject="", $message="", $file='')
		{
			...
			if($this->optcompany['mailertype'] == 1)
		    {
				...
				$mail->SetFrom($mail->Username,$this->company['tenvi']);
		    }
