										...
										<div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
											...
											<?php if(isset($config['product'][$type]['motangan']) && $config['product'][$type]['motangan'] == true) { ?>
												<div class="form-group">
													...
													<textarea class="form-control for-seo <?=(isset($config['product'][$type]['motangan_cke']) && $config['product'][$type]['motangan_cke'] == true)?'form-control-ckeditor':''?>" name="data[motangan<?=$k?>]" id="motangan<?=$k?>" rows="5" placeholder="Mô tả ngắn (<?=$k?>)"><?=htmlspecialchars_decode(@$item['motangan'.$k])?></textarea>
												</div>
											<?php } ?>
